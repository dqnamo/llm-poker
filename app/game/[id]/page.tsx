            ...
            <div className="text-neutral-200  grid lg:grid-cols-3 gap-8 max-w-7xl mx-auto w-full">
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <h1 className="text-sm font-semibold font-sans">
                      Game{" "}
                      <span className="text-neutral-500 font-semibold text-xs uppercase">
                        {gameId.slice(-8)}
                      </span>
                    </h1>
              ...
              <div className="grid grid-cols-3 relative col-span-3 lg:col-span-3 space-x-2">
                ...
                <Player
                  ...
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.hands.filter(
                      (hand: hand) => hand.player[0]?.id === game.players[0]?.id
                    )[0]?.cards.cards
                  }
                ...
                <Player
                  ...
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.hands.filter(
                      (hand: hand) => hand.player[0]?.id === game.players[1]?.id
                    )[0]?.cards.cards
                  }
                ...
                <Player
                  ...
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.hands.filter(
                      (hand: hand) => hand.player[0]?.id === game.players[2]?.id
                    )[0]?.cards.cards
                  }
                ...
                <Table
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.communityCards
                      .cards
                  }
                ...
                <Player
                  ...
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.hands.filter(
                      (hand: hand) => hand.player[0]?.id === game.players[5]?.id
                    )[0]?.cards.cards
                  }
                ...
                <Player
                  ...
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.hands.filter(
                      (hand: hand) => hand.player[0]?.id === game.players[4]?.id
                    )[0]?.cards.cards
                  }
                ...
                <Player
                  ...
                  cards={
                    game.gameRounds[game.gameRounds.length - 1]?.hands.filter(
                      (hand: hand) => hand.player[0]?.id === game.players[3]?.id
                    )[0]?.cards.cards
                  }
...
const Player = ({
  ...
  return (
    <div
      ...
      className={`p-px overflow-hidden relative rounded-md h-full ${
    ...
    >
      <div
        className={`bg-neutral-950 relative hover:bg-neutral-900 flex flex-col h-full rounded-md`}
      >
        {/* Player info section - fixed height */}
        <div className="flex flex-col lg:flex-row items-start gap-2 justify-between p-4">
          <div className="flex flex-col h-full">
            <div className="text-xs font-semibold font-sans capitalize">
              {player?.name}
            </div>
            ...
            <div
              className={`absolute rounded-full top-4 right-4 h-3 w-3 ${
                button ? "bg-white" : "bg-transparent"
              }`}
            ></div>
    ...
    <div className="col-span-6 relative perspective-[1000px] mb-8 w-[95%] mx-auto">
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center relative z-10 min-h-[120px]">
          ...
          <div
            className={`flex flex-row items-center gap-1 bg-green-950/50 rounded-md px-4 py-2 ${
              Number(pot) > 0 ? "opacity-100" : "opacity-0"
            }`}
          >
            ...
            {Array.from({ length: 5 }).map((_, index) =>
              cards && cards[index] ? (
                <Card
                  key={cards[index]}
                  value={cards[index]}
                  className="w-8 h-11 sm:w-10 sm:h-14 transform-gpu"
                />
              ) : (
                <div
                  key={`placeholder-${index}`}
                  className="w-8 h-11 sm:w-10 sm:h-14 bg-transparent"
                ></div>
              )
            )}
          </div>

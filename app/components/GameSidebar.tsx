...
interface GameSidebarProps {
  game: InstaQLEntity<AppSchema, "games"> & {
    ...
    gameRounds?: Array<InstaQLEntity<AppSchema, "gameRounds"> & {
      ...
      hands: Array<InstaQLEntity<AppSchema, "hands"> & {
        player: { id: string }[];
        cards: { cards: string[] };
      }>;
    }>;
...
const CornerBorders = () => {
  return (    
    <>  
    <div className="border-r-3 border-t-3 border-neutral-700 h-3 w-3 absolute -top-1 -right-1"/>
    <div className="border-l-3 border-b-3 border-neutral-700 h-3 w-3 absolute -bottom-1 -left-1"/>
    <div className="border-l-3 border-t-3 border-neutral-700 h-3 w-3 absolute -top-1 -left-1"/>
    <div className="border-r-3 border-b-3 border-neutral-700 h-3 w-3 absolute -bottom-1 -right-1"/>
    </>
  );
};

export default function GameSidebar({ game, selectedPlayer, onPlayerSelect }: GameSidebarProps) {
  const [activeTab, setActiveTab] = useState<'analytics' | 'thinking' | 'rankings'>('rankings');
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-neutral-900/50 rounded-xl border border-neutral-800/50">
      ...
      <div className="overflow-y-auto">
        {activeTab === 'analytics' && <AnalyticsTab game={game} chartData={chartData} />}
        {activeTab === 'thinking' && <ThinkingTab selectedPlayer={selectedPlayer} onPlayerSelect={onPlayerSelect} game={game} />}
    ...
    <div className="p-4 space-y-6">
      ...
      <div className="bg-neutral-950 border border-neutral-800 relative">
        ...
        <div className="p-3 border-b border-neutral-800">
          <h3 className="text-xs font-medium uppercase text-neutral-200">Balances Over Time</h3>
      ...
      <div className="space-y-4">
        <div className="bg-neutral-950 border border-neutral-800 relative">
          <CornerBorders />
          <div className="p-3 border-b border-neutral-800">
            <h3 className="text-xs font-medium uppercase text-neutral-200">Game Details</h3>
          </div>
        ...
        <div className="bg-neutral-950 border border-neutral-800 relative">
          <CornerBorders />
          <div className="p-3 border-b border-neutral-800">
            <h3 className="text-xs font-medium uppercase text-neutral-200">Transaction Summary</h3>
          </div>
          <div className="p-3 space-y-2">
            {(() => {
                ...
                <>
                  <div className="flex justify-between text-xs">
                    ...
                    <div className="flex items-center gap-1">
                      <DiamondsFourIcon size={10} className="text-green-500" weight="fill" />
                  ...
                  <div className="flex justify-between text-xs">
                    ...
                    <div className="flex items-center gap-1">
                      <DiamondsFourIcon size={10} className="text-red-500" weight="fill" />
...
function ThinkingTab({ selectedPlayer, onPlayerSelect, game }: { selectedPlayer: PlayerWithRelations | null, onPlayerSelect: (player: PlayerWithRelations | null) => void, game: GameSidebarProps['game'] }) {
  if (!selectedPlayer) {
    return (
      <div className="p-4">
        <div className="flex flex-col items-center justify-center h-40 text-center">
          <Brain size={32} className="text-neutral-600 mb-2" />
          <p className="text-xs text-neutral-500 uppercase font-medium">Select a Player</p>
          <p className="text-xs text-neutral-600 mt-1">Click on any player to view their AI thinking process</p>
        </div>
      </div>
    );
  }
  ...
  const actionsByRound = gameRounds.map((round) => {
    const roundActions = playerActions.filter((action) => action.gameRound?.id === round.id);
    return {
      round,
  ...
  const totalWinnings = selectedPlayer.transactions?.reduce((acc, tx) => {
    ...
    <div className="relative h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between p-4 border-b border-neutral-800 bg-neutral-950">
        <div className="flex items-center gap-2">
          <h3 className="text-xs font-semibold uppercase">{selectedPlayer.name}</h3>
        </div>
        <button
      ...
      <div className="relative">
        ...
        <div className="p-4 border-b border-neutral-800">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center">
              <div className="text-xs text-neutral-500 uppercase mb-1">Stack</div>
              <div className="flex items-center justify-center gap-1">
                <DiamondsFourIcon size={12} className="text-green-500" weight="fill" />
            ...
            {/* Total Winnings */}
            <div className="text-center">
              <div className="text-xs text-neutral-500 uppercase mb-1">Winnings</div>
              <div className="flex items-center justify-center gap-1">
                {totalWinnings >= 0 ? (
                  <>
          ...
          {currentHand?.cards?.cards && currentHand.cards.cards.length > 0 && (
            <div className="mt-4">
              <div className="text-xs text-neutral-500 uppercase text-center mb-2">Current Hand</div>
        ...
        {selectedPlayer.notes && (
          <div className="p-4 border-b border-neutral-800">
            <h4 className="text-xs font-medium uppercase text-neutral-200 mb-2">AI Observations</h4>
        ...
        <div className="p-4 overflow-y-auto">
          <h4 className="text-xs font-medium uppercase text-neutral-200 mb-3">Action Timeline</h4>
          
          <div className="space-y-4">
            {actionsByRound.reverse().map((roundGroup, roundIndex) => (
              <div key={roundGroup.round.id} className="bg-neutral-950 border border-neutral-800 relative">
                <CornerBorders />
                <div className="p-3">
                    ...
                    {roundGroup.round.communityCards?.cards && roundGroup.round.communityCards.cards.length > 0 && (
                      <div className="flex gap-1">
                        {roundGroup.round.communityCards.cards.map((card: string) => (
                          <Card key={card} value={card} className="w-4 h-5" />
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    {roundGroup.actions.map((action) => (
                      <div key={action.id} className="space-y-2">
                        ...
                        {action.reasoning && (
                          <div>
                            <div className="text-xs text-neutral-600 mb-1">Reasoning:</div>
            ...
            {actionsByRound.length === 0 && (
              <div className="flex items-center justify-center py-8">
                <p className="text-xs text-neutral-500">No actions recorded yet</p>
              </div>
            )}
          </div>
        </div>
...
function RankingsTab({ players }: { players?: PlayerWithRelations[] }) {
  ...
  return (
    <div className="p-4">
      <div className="mb-3">
        <h3 className="text-xs font-medium uppercase text-neutral-200">Winnings</h3>
        <p className="text-xs text-neutral-500">How the models are doing</p>
      </div>
      
      <Reorder.Group as="div" axis="y" values={rankedPlayers} onReorder={setRankedPlayers} className="space-y-2">
